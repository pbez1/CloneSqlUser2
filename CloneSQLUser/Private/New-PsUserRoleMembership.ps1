...
#>
function New-PsUserRoleMembership {
    [cmdletbinding()]
    ...
    Write-Verbose "Add user's database role membership..."

    $perms = "`n-------------------------------------------------------------------------------`n"
    $perms += "-- Add user's database role membership`n"
    $perms += "-------------------------------------------------------------------------------`n"
    ...
    try{
        foreach ($database in $Databases) {
            ...
            # Verify if the user is a member of any roles.
            $sql = "
                select 1 is_present 
                from
                    sys.database_principals roles
                    inner join sys.database_role_members mbr on roles.principal_id = mbr.role_principal_id and roles.type = 'R'
                where 
                    lower(user_name(mbr.member_principal_id)) = lower('$FromUser')
                "
            ...
            if ($user_is_present) {
                ...
                $sql = "
                    use [$database]
                    select
                        'alter role [' + roles.name + '] add member [$ToUser]' + char(13) + char(10) script
                    from
                        sys.database_principals roles
                        inner join sys.database_role_members mbr on roles.principal_id = mbr.role_principal_id
                        inner join sys.database_principals users on mbr.member_principal_id = users.principal_id
                    where
                        roles.type not in ('G', 'U', 'S')
