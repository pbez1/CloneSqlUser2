...
#>
function New-PsRoleSchemaPermissions {
    [cmdletbinding()]
    ...
    Write-Verbose "Add role's schema permissions..."

    $perms = "`n-------------------------------------------------------------------------------`n"
    $perms += "-- Add role's schema permissions`n"
    $perms += "-------------------------------------------------------------------------------`n"
    ...
    try{
        foreach ($database in $Databases) {
            ...
            # then those permissions should be rebuilt if they're not already present.

            # Get a list of the roles the user is a member of.
            $sql = "
                select
                    roles.name role
                from
            ...
            foreach ($role in $roles) {
                # Verify if the role has permissions on any schema.
                $sql = "
                    select 
                        1 is_present
                    ...
                    "
                $role_is_present = (invoke-sqlcmd @sql_parms -ServerInstance $SourceServer -Database $database -Query $sql).is_present

                if ($role_is_present) {
                    $perms += "use [$database]`n"
                    ...
                    $sql = "
                        ...
                        where
                            class_desc = 'SCHEMA'
                            and user_name(grantee_principal_id) = '$role'
                        "
